        ...
        for kfm = 1:size(fminmaxArr, 1)
            for kw = 1:length(MotherWaveletArr)
                for ksd = 1:length(signalDerivationArr)
                    for kno = 1:length(noiseLevelArr)
                        Q = Qarr(kq);
                        ...
                        for kc = 1:length(ridgeContinuityArr)
                            ...
                            % CWT
                            if isempty(CWT)
                                AcaptPower = mean(Acapt.^2);
                                AcaptNoise = Acapt + sqrt(AcaptPower/10^(noiseLevel/10)) * randn(size(Acapt));
                                CWT = WvltComp(S.T, AcaptNoise, freqs, Q, 'MotherWavelet', MotherWavelet,...
