...
for k_tour = 1:12
    fprintf('~~~~ tour %d ~~~~\n', k_tour);
    [comb, esp, comb_possible] = meilleureComb(combinaisons_init, combinaisons, n_c, n_p);
    if comb_possible
        comb_possible = 'oui';
    else
        comb_possible = 'non';
    end
    disp([sprintf('coup à jouer : (%d combinaisons restantes, %.1f%% d''elimination, combinaison possible : ',...
        [size(combinaisons, 1), esp*100]), comb_possible, ')']);
...
function [comb, esperance, comb_possible] = meilleureComb(combinaisons_init, combinaisons, n_c, n_p)
...
comb_possible = any(ind_combs_in_combinaisons);
if comb_possible
